...
void    draw_full_map(t_map *full_map, t_map_objects *map_objs, t_player *player)
{
    ...
    while (map_objs->map[i] != NULL)
    {
        ...
        {
			draw_cell(full_map, x, y, get_cell_colour(map_objs->map[i][j]), full_map->cell_size);
	...
	x = full_map->map_x + (player->x * full_map->cell_size) - full_map->cell_size / 4;
	y = full_map->map_y + (player->y * full_map->cell_size) - full_map->cell_size / 4;
	draw_cell(full_map, x, y, MAP_PLAYER, full_map->cell_size / 2);
}
